...
class BootstrapLinuxHeadersSeed(Package):
    ...
    A verbatim copy of the builtin ``linux-libre-headers-bootstrap``: Guix's
    ``%bootstrap-linux-libre-headers``, the sanitized, sha256-pinned
    ``linux-libre-headers-stripped-4.14.67-i686-linux`` tarball, unpacked as-is.
    No compiler and no ``make`` are involved, so the seed compiler (tcc-mes) is
    never asked to build ``fixdep``/``unifdef``.
    ...
    like the builtin ``linux-headers-boot0``) supplies x86_64 headers to the
    pristine musl + later gcc stages.

    Note on arch: Guix ships no x86_64 bootstrap seed (its whole early bootstrap
    is i686, capped to x86_64 later), so these are i686 4.14.67 headers. musl
    carries its own per-arch syscall numbers and uses kernel headers mainly for
    (largely x86-unified) structs/ioctls, so this suffices for the throwaway
    scaffold; the real x86_64 headers arrive with bootstrap-linux-headers.

    No ``c`` dependency. Ported from the builtin linux-libre-headers-bootstrap
    (Guix ``gnu/packages/bootstrap.scm``, ``%bootstrap-linux-libre-headers``)."""
